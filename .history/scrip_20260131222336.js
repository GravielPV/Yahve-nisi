...
function initializeCarousel() {
  ...
  // Remove old onclick events if any, though addEventListener stacks.
  // We assume HTML will be cleaned.
...
function initializeForm() {
  ...
  form.addEventListener("submit", function (e) {
    ...
    const selectedService =
      serviceNames[service] || service || "No especificado";

    // Create email content
    const subject = encodeURIComponent(
      `Consulta Web - ${name} - ${selectedService}`,
    );
    ...
    // Loading state
    submitBtn.innerHTML =
      '<i class="fas fa-spinner fa-spin" style="margin-right: 0.5rem;"></i>Abriendo email...';
    ...
    try {
      ...
      setTimeout(() => {
        submitBtn.innerHTML =
          '<i class="fas fa-check" style="margin-right: 0.5rem;"></i>¡Email Abierto!';
        submitBtn.style.background =
          "linear-gradient(135deg, #10b981, #059669)";

        // Show success message
        showNotification(
          "Se ha abierto tu cliente de email. Por favor, envía el mensaje.",
          "success",
        );
      ...
      showNotification(
        `Error al abrir email. Copia esta información: ${recipientEmail}`,
        "error",
      );
...
if (whatsappBtn) {
  whatsappBtn.addEventListener("click", function () {
    ...
    const message = encodeURIComponent(
      "¡Hola! Me interesa conocer más sobre sus servicios de impresión. ¿Podrían proporcionarme más información?",
    );
